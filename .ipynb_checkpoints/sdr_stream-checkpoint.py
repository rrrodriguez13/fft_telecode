...
async def _collate_streams(q, sdrs, nblocks, nsamples, callback):
    ...
    try:
        while True:
            ...
            cnts[dev_id] += 1
            q.task_done()
            # When we get a full block, reset counter and then process block
            if cnts[dev_id] >= nblocks:
                cnts[dev_id] = 0
                # Process the block that just filled
                callback(dev_id, shape, data[dev_id])
...
def default_callback(dev_id, shape, data):
    print(f'Got data {dev_id}, {shape}, {data.dtype}')

def capture_data(sdrs, nsamples=2048, nblocks=1, callback=default_callback):
    """
